...
- **中央カラム（記入・時系列）**: パスコード→カルテ番号→動物名確認→記入者選択→記入…を同じレイアウト内の状態遷移として実施。
  - 記入者は13名を横一列・単一選択。
  ...
  - 記録は**追記型**（上書きしない）。訂正は新しい記録の追記で行う運用。誤入力の削除と★の切り替えのみ可能。
...
│   ├── db.js                 # Realtime Databaseへの読み書き（エントリ・定型文・検査予定・薬剤・既往歴）
