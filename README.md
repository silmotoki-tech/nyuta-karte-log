...
- **Firebase ログイン**: 起動時に院内共通アカウント（メール/パスワード）。セッションは端末に永続化。
- **パスコード認証**: Firebase ログイン後に4桁パスコード（`2211`）。当日中は再入力不要。
...
- **右カラム（タブ切替）**: 既往歴／検査予定／薬剤情報／処置ログ／自由質問。
  ...
  - **薬剤情報（実装済み）**: 薬剤マスタから選択（手打ち可）、出来事履歴（継続/一時的/投与難/増量/減量/休薬中/再開/中止・編集/削除可）、投与頻度（よくあるパターン／○日に○回／週○回／曜日指定／その他）、増減時の量プリセット、カテゴリA/B/C、副作用メモ、処方切れ目安アラート。使用状況（継続/一時的/投与難/休薬中/中止）は最新出来事から自動導出。データは `medications/{カルテ番号}/{drugId}`（`schemaVersion` 付き）。
...
- **Firebase Authentication（メール/パスワード）**（DB読み書きはログイン完了後に実行）。院内展開手順は `docs/AUTH-ROLLOUT.md`。
...
Firebaseコンソールの「Authentication」→「Sign-in method」で **メール/パスワード** を有効にし、院内共通アカウントを1つ作成してください。

院内の稼働端末への切り替えは順番が重要です（匿名認証をすぐ無効化しない・UID 限定ルールを急いでデプロイしない）。詳細は **`docs/AUTH-ROLLOUT.md`** を必ず読んでください。

### 3. Realtime Databaseのルール

リポジトリの `database.rules.json` が定義です。展開完了後は院内共通アカウントの UID のみ読み書き可にします。

```bash
firebase deploy --only database
```

※ ルール変更はデータの削除にはなりません（権限のみ）。展開タイミングは `docs/AUTH-ROLLOUT.md` のステップ5です。

### アプリ内パスコードについて

Firebase ログインのあと、4桁のパスコード（`js/app.js` 内の `PASSCODE` 定数、初期値 `2211`）の入力を求めます。正しく入力すると `localStorage` に認証フラグと認証日付が保存され、同じ日のうちは再入力不要です。設定メニューの「ログアウト」でパスコードのみ解除、「アカウントからサインアウト」で Firebase からも抜けます（後者は管理者パスコード保護付き）。
  ...
  {カルテ番号}/
    {drugId}/
      ...
      events/
        {eventId}/
          date: "2026-07-18"
          type: "decrease"                   # add|temporary|hard|increase|decrease|hold|stop|resume
          detail: "食欲低下のため"
